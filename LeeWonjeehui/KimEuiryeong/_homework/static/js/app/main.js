...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function addBotMessage(answer) {
    const botHTML = `
      <div class="chat-bot">
        <div class="chat-bot__content">${answer}</div>
  ...
  function sendChatMessage() {
    ...
    if (message !== '') {
      ...
      fetch('/chat_api/', {
        ...
        body: formData  // 이제 정의된 formData 사용
      })
      .then(response => {
        console.log('응답 상태:', response.status);
        if (!response.ok) {
      ...
      .then(data => {
        console.log('받은 전체 응답:', data);
        console.log('받은 실제 답변:', data.answer);
        
        const answer = data.answer || data.response || '답변을 받지 못했습니다.';
        addBotMessage(answer);
      })
      .catch(error => {
        console.error('에러 발생:', error);
        addBotMessage('⚠️ 서버 오류 발생: ' + error.message);
      });
