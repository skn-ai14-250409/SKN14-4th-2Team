...
def chat_api(request):
    ...
    print(f"요청 데이터: {request.POST}")  # 디버깅용
    
    if request.method == 'POST':
        try:
            # FormData 사용시 request.POST에서 데이터 가져오기
            question = request.POST.get('question')  # json.loads 대신 POST 직접 접근
