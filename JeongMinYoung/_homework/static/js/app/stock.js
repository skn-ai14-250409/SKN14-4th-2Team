...
function formatMarkdown(message) {
    ...
    try {
        ...
        if (typeof marked !== 'undefined') {
            ...
            // 추가 스타일링 클래스 적용
            html = enhanceMarkdownStyling(html);
...
function enhanceMarkdownStyling(html) {
    // 테이블에 부트스트랩 클래스 추가
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (companyName) {
        try {
            ...
            console.log(`stock.js: URL 파라미터로 받은 기업명: "${companyName}" → 디코딩: "${decodedCompanyName}"`);
            
            // 검색창에 기업명 표시
            const searchInput = document.getElementById('stockSearchInput');
            const headerSearchInput = document.getElementById('headerStockSearchInput');
            if (searchInput) {
                searchInput.value = decodedCompanyName;
                console.log(`stock.js: 검색창에 값 설정 - "${decodedCompanyName}"`);
            }
            if (headerSearchInput) {
                headerSearchInput.value = decodedCompanyName;
            }
            
            // 기존의 완전한 검색 로직 사용 (fetchStockData)
            setTimeout(() => {
                console.log(`stock.js: 기존 fetchStockData 함수로 검색 실행 - "${decodedCompanyName}"`);
                fetchStockData(decodedCompanyName, '1일', true); // 기존 함수 호출
            }, 500); // 페이지 로드 후 0.5초 대기
        } catch (error) {
            console.error('stock.js: URL 파라미터 디코딩 오류:', error);
            console.log(`stock.js: 원본 파라미터로 검색 시도: "${companyName}"`);
            setTimeout(() => {
                fetchStockData(companyName, '1일', true); // 기존 함수 호출
            }, 500);
        }
    }
...
function showLoadingMessage() {
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                try {
                    ...
                    // RAG 분석 실행 (최초 검색 시에만, 기간 변경 시에는 제외)
                    if (period === '1일') {  // 최초 검색 시에만 분석 실행
                        console.log(`stock.js: RAG 분석 시작 - ${companyName}`);
                        rag(companyName);
                    }
                } catch (error) {
    ...
    function rag(title) {
        ...
        rag_result.innerHTML = '<div class="loading-message"><i class="bi bi-hourglass-split"></i> 분석중입니다...</div>'
        ...
        .then((response) => {
          ...
          .then((data) => {
            let answer = data['answer'] || "분석 결과가 없습니다.";
            
            // 제목을 동적으로 변경 (하드코딩된 제목을 기업명으로 교체)
            // 다양한 형태의 제목 패턴을 처리
            answer = answer.replace(/([가-힣A-Za-z]+(?:\s[가-힣A-Za-z]+)*)\s+\d+년\s+\d+분기\s+리서치\s+보고서/g, `${title} 리서치 보고서`);
            answer = answer.replace(/#\s*([가-힣A-Za-z]+(?:\s[가-힣A-Za-z]+)*)\s+\d+년\s+\d+분기\s+리서치\s+보고서/g, `# ${title} 리서치 보고서`);
            
            // 추가: 마크다운이 아닌 일반 제목 패턴도 처리
            answer = answer.replace(/^([가-힣A-Za-z]+(?:\s[가-힣A-Za-z]+)*)\s+\d+년\s+\d+분기\s+리서치\s+보고서/m, `${title} 리서치 보고서`);
            
            // 마크다운 렌더링 적용
            rag_result.innerHTML = formatMarkdown(answer);
            console.log('stock.js: RAG 분석 결과 마크다운 렌더링 완료');
          })
          .catch((error) => {
            console.error("에러 발생:", error);
            rag_result.innerHTML = '<div class="error-message"><i class="bi bi-x-circle"></i> 서버 오류가 발생했습니다. 다시 시도해주세요.</div>';
          });
    };
...
// 자동 주식 검색 함수 (URL 파라미터로 받은 기업명으로 검색)
async function autoSearchStock(companyName) {
    console.log(`stock.js: 자동 검색 시작 - ${companyName}`);
    ...
    try {
        ...
        console.log('stock.js: 자동 검색 응답 데이터:', data);
        ...
        if (data.success) {
            ...
            if (headerSearchInput) headerSearchInput.value = companyName;
            
        } else {
            console.error('stock.js: 자동 검색 실패:', data.error);
            showErrorState(`${companyName} 정보를 찾을 수 없습니다.`);
        }
        
    } catch (error) {
        console.error('stock.js: 자동 검색 오류:', error);
        showErrorState('서버와의 연결에 문제가 발생했습니다.');
    }
}
...
function showLoadingState() {
    ...
    if (loadingMessage) {
        ...
        loadingMessage.innerHTML = '<i class="bi bi-hourglass-split"></i> 데이터를 불러오는 중...';
    }
}
...
function showErrorState(message) {
    ...
    if (loadingMessage) {
        loadingMessage.style.display = 'block';
        loadingMessage.innerHTML = `<i class="bi bi-x-circle"></i> ${message}`;
    }
}

// 주식 데이터 표시
function displayStockData(data) {
    console.log('stock.js: 주식 데이터 표시:', data);
    ...
    if (stockInfoDisplay) {
        ...
        if (stockSymbol) stockSymbol.textContent = data.code;
        if (currentPrice) currentPrice.textContent = `₩${data.latestPrice}`;
        if (updateTime) updateTime.textContent = `${new Date().toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit' })} KST`;
        ...
        if (priceChange) {
            ...
            priceChange.textContent = `${arrow} ${data.priceChange.replace(/[+-]/, '')} (${data.changePercent.replace(/[+-]/, '')}%)`;
        }
        ...
        if (fiftyTwoWeekHigh) fiftyTwoWeekHigh.textContent = data.fiftyTwoWeekHigh || '--';
        if (fiftyTwoWeekLow) fiftyTwoWeekLow.textContent = data.fiftyTwoWeekLow || '--';
        if (dayHigh) dayHigh.textContent = data.dayHigh || '--';
        if (dayLow) dayLow.textContent = data.dayLow || '--';
